...
int main() {
    ...
    LCA D(n, 0, adj);
    vector<int> size(n), parent(n, -1), is_centroid(n, 0), color(n, 0);
    vector<set<pair<int, int>>> ans_down(n);
    ...
    function<void(int)> update = [&](int v) {
        if (color[v] == 0) {
            int u = v; color[v] = 1;
            while (u != -1) {
                ans_down[u].emplace(D.get(u, v), v);
                u = parent[u];
            }
        } else {
            int u = v; color[v] = 0;
            while (u != -1) {
                ans_down[u].erase({D.get(u, v), v});
                u = parent[u];
            }
        }
    };
    function<int(int)> get = [&](int v) {
        ...
        while (u != -1) {
            if (!ans_down[u].empty()) {
                ans = min(ans, (ans_down[u].begin() -> first) + D.get(u, v));
            }
            u = parent[u];
        }
        return (ans == n ? -1 : ans);
